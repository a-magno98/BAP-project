...
df_groupby.columns = ['User', 'Information', 'Diff_Time']
...
subset.to_csv("plot_distr_Theory.csv", index=False)
#######################################################################################

#filter selecting the quiz
df2 = df2[df2.Information == 'Quiz']
df2_groupby = df2.groupby(['User', 'Information'])['Diff_Time'].sum()
df2_groupby.columns = ['User', 'Information', 'Diff_Time']
...
subset2.to_csv("plot_distr_Quiz.csv", index=False)
################################################################################

#filter selecting time spent in forum
df3 = df3[df3.Information == 'Forum']
df3_groupby = df3.groupby(['User', 'Information'])['Diff_Time'].sum()
df3_groupby.columns = ['User', 'Information', 'Diff_Time']
...
subset3.to_csv("plot_distr_Forum.csv", index=False)
###################################################################################

#filter selecting the resource which means time spent in assignment
df4 = df4[df4.Information == 'Assignment']
df4_groupby = df4.groupby(['User', 'Information'])['Diff_Time'].sum()
df4_groupby.columns = ['User', 'Information', 'Diff_Time']
df4_groupby = df4_groupby.reset_index()
df4_groupby = df4_groupby.rename(columns={'Diff_Time':'Time_Assignment'})

#calculate the time spent on Assignment in hours
df4_groupby['Time_Assignment']=[(int)(df4_groupby.Time_Assignment[i].total_seconds()//3600) for i in range(len(df4_groupby['Time_Assignment']))] 

cols4 = ['User', 'Time_Assignment']
subset4 = pd.DataFrame(df4_groupby, columns = cols4)

subset4.to_csv("plot_distr_Assignment.csv", index=False)
###################################################################################
